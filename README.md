...
IP Weave 采用 Agent 加工具的架构。Agent 是总指挥，负责决策和调度，不亲自执行具体任务。工具是独立的功能模块，每个工具负责一个特定领域的工作。

Agent 层：启动时先由 GLM-5.1 分析任务目标并将其拆解为多个可执行的步骤。执行过程中每轮读取当前状态，决定下一步调用哪个工具。执行后评估结果，判断是否需要迭代修正。

工具层：当前注册了八个工具。read_ip 读取链上数据，analyze_style 分析风格特征，write_story 创作衍生故事，write_script 生成动画脚本，design_assets 设计周边资产，render_3d 生成 3D 模型，check_quality 检查风格一致性，deliver 生成交付物。

数据流：Agent 收到 IP 名称后调用 read_ip 读取链上数据，将结果存入状态对象。然后调用 analyze_style 分析风格。接着依次调用 write_story、write_script、design_assets 生成内容。如有需要调用 render_3d 生成 3D 模型。全部完成后调用 check_quality 检查质量。最后调用 deliver 生成 HTML 报告和 NFT 合约。
...
具体目标包括：第一，让用户只需要输入一个链上 IP 名称或合约地址，Agent 自主完成全部创作流程，不需要任何专业技能。第二，确保所有产出内容在视觉和叙事风格上与原始 IP 保持一致。第三，将衍生内容打包为 NFT 并部署到测试网，实现从创意到上链的完整闭环。

---

## GLM-5.1 调用位置与关键流程

GLM-5.1 在整个系统中扮演核心决策和创作角色，以下是各个调用点：

任务拆解阶段 ( 方法)：GLM-5.1 分析用户输入的 IP 名称和任务目标，将其拆解为多个可执行步骤并排定优先级。调用方式为 chat_json，返回结构化 JSON 格式的执行计划。

状态决策阶段 ( 方法)：GLM-5.1 读取当前状态对象中的数据，分析哪些步骤已完成、哪些需要改进、下一步最应该做什么。调用方式为 chat_json，返回包含动作名称和决策理由的 JSON。

风格分析阶段 (StyleAnalyzer.extract 方法)：GLM-5.1 根据链上数据提取 IP 独有的视觉风格、叙事基调、文化背景和代表符号。调用方式为 chat_json，返回结构化风格画像。

故事创作阶段 (StoryGenerator.generate 方法)：GLM-5.1 根据风格画像和创作计划生成完整的衍生故事。调用方式为 chat，返回 2000 到 4000 字的叙事文本。

脚本生成阶段 (ScriptGenerator.generate 方法)：GLM-5.1 将故事文本转化为专业的分镜脚本。调用方式为 chat，返回包含镜号、画面描述、对白、镜头运动和时长的结构化脚本。

资产设计阶段 (AssetDesigner.generate 方法)：GLM-5.1 根据风格画像设计周边资产概念。调用方式为 chat_json，返回包含产品名称、类型、描述、设计和 NFT 元数据的 JSON。

质量检查阶段 (StyleChecker.check 方法)：GLM-5.1 评估生成内容与 IP 风格画像的匹配度。调用方式为 chat_json，返回包含评分和修改建议的 JSON。
