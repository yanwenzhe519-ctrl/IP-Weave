...
### 自动执行
Agent 在收到 IP 名称后自动执行全部流程，无需人工干预。每步都有日志输出，用户可在终端实时查看。Agent 循环上限 20 轮，超时自动终止交付当前成果。

### 权限
Agent 仅具有链上数据只读权限（读取 ERC-721 元数据）。合约部署使用临时生成的测试钱包，私钥仅在内存中使用，不会写入磁盘或上传。

### 成本
GLM-5.1 API 通过 Coding Plan 订阅计费。合约部署到 Sepolia 测试网，测试网 ETH 通过公开水龙头免费获取，无实际经济价值。CogView-4 出图需单独购买额度，不影响核心流程。

### 外部工具
所有第三方 API 和 SDK 均在 README 中列出说明。GLM-5.1 和 CogView-4 为智谱 AI 商业 API，用户需自行注册获取密钥。build123d 为开源库。

### 钱包
Agent 使用预置的测试网钱包，仅限 Sepolia 测试网，不涉及主网和真实资产。用户可自行替换为自己的测试网钱包私钥。

### 链上操作
所有链上操作仅限于 Sepolia 测试网。不支持主网操作。交易记录可在 Etherscan 上查看。

### 自动发布
Agent 完成内容生成后自动调用合约部署和 NFT 铸造。可在代码中注释 deploy_to_sepolia 调用以禁用自动上链。

### 失败处理
API 调用超时自动重试一次。CogView-4 出图失败不影响核心文本生成。合约部署失败记录错误日志，不会自动重试消耗 Gas。Agent 20 轮循环后自动终止。

### 人工介入
用户可按 Ctrl+C 随时中断执行。关键步骤有日志输出。合约部署前打印 Gas 费用估算，用户可确认后继续。
...
第一阶段（1-2 个月）：接入 CogVideoX 或 Seedance 2.0 实现视频动画生成，让 Agent 能直接产出动画短片。增加更多内容类型，如音乐生成和 3D 场景包。
...
SaaS 订阅模式：为 NFT 项目方提供月度内容运营服务，按 IP 数量和使用频次收费。

单次创作服务：为个人持有者提供单次衍生内容生成服务，输入 IP 名称即可获得完整衍生内容包。
...
全链路闭环：市场上唯一能同时完成故事、脚本、资产、3D 模型并自动上链的产品。

成本优势：传统成本 5-20 万元，IP Weave 降低到几乎为零。

风格统一：GLM-5.1 深度分析保证风格一致。
