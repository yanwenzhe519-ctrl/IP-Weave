...
以处理 Azuki IP 为例，Agent 的实际决策过程如下：先读取链上数据，数据为空时自主决定调用 read_ip 工具从以太坊主网获取链上数据；数据读取完成后判断需要分析风格特征，调用 analyze_style 工具分析视觉风格和叙事基调；风格分析完成后认为前置条件已满足，决定创作衍生故事，调用 write_story 工具生成 3000 字以上的完整叙事；故事创作完成后调用 write_script 将故事转化为分镜脚本；然后调用 design_assets 设计周边资产、render_3d 生成 3D 模型、check_quality 检查风格一致性、deliver 交付。每次决策都基于当前实际状态，不是预设的固定顺序。
